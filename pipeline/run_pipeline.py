...
def check_model_parameters(model):
    for name, param in model.named_parameters():
        if torch.isnan(param).any() or torch.isinf(param).any():
            ...
            print(f"Max value: {param.max().item()}, Min value: {param.min().item()}")
            print(
                f"Mean value: {param.mean().item()}, Std deviation: {param.std().item()}"
            )
...
def run_pipeline(model_path):
    ...
    check_model_parameters(model_base.model)
    ...
    # Print sample inputs
    print("Sample harmful instruction:", harmful_train[0])
    print("Sample harmless instruction:", harmless_train[0])
    ...
    try:
        ...
        print(f"Error in generate_and_save_candidate_directions: {str(e)}")
        print("Traceback:")
        import traceback
